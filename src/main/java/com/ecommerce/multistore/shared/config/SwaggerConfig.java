package com.ecommerce.multistore.shared.config;

import io.swagger.v3.oas.models.OpenAPI;
import io.swagger.v3.oas.models.info.Info;
import io.swagger.v3.oas.models.info.Contact;
import io.swagger.v3.oas.models.info.License;
import io.swagger.v3.oas.models.servers.Server;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

import java.util.List;

/**
 * إعدادات Swagger/OpenAPI
 * Swagger/OpenAPI Configuration
 * 
 * @author Multi-Store Team
 * @version 1.0
 */
@Configuration
public class SwaggerConfig {

    /**
     * إعداد OpenAPI للتوثيق
     * OpenAPI configuration for documentation
     * 
     * @return OpenAPI instance
     */
    @Bean
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        .title("Multi-Store E-commerce API")
                        .description("API متكامل للمتجر الإلكتروني متعدد المجالات - A comprehensive API for multi-domain e-commerce platform")
                        .version("1.0.0")
                        .contact(new Contact()
                                .name("Multi-Store Team")
                                .email("support@multistore.com")
                                .url("https://multistore.com"))
                        .license(new License()
                                .name("MIT License")
                                .url("https://opensource.org/licenses/MIT")))
                .servers(List.of(
                        new Server()
                                .url("http://localhost:8080")
                                .description("Development Server"),
                        new Server()
                                .url("https://api.multistore.com")
                                .description("Production Server")));
    }
}
